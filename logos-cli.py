...
# Pedir datos de entrada por teclado al usuario.

print("Operadores disponibles: not(p), and(p,q), or(p,q) if(p,q), iff(p,q)," +
        "nr(p,q) y xr(p,q).")

nv = int(input("Ingrese el número total de variables: "))

var = [ ]

for i in range(nv):
    var.append(input("Ingrese símbolo de la variable #" + str(i + 1) + ": "))
...
print("Para terminar la entrada de premisas, ingrese 0.")
...
conc = input("Ingrese una fórmula booleana (conclusión): ")

# Verificar que el argumento es válido.

if isvalidarg(prems,conc,var):
